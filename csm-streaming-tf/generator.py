...
def run_warmup(generator):
    ...
    inputs = generator.processor.apply_chat_template(
        [{
            "role": "0",
            "content": [
                {"type": "text", "text": "This is a warmup generation to make the model super fast."}
            ]
        }],
        tokenize=True,
        return_dict=True
    ...
    for i, chunk in enumerate(generator.generate_stream(inputs, chunk_token_size=20)):
        ...
        print(f"Generated warmup chunk {i+1} with {len(chunk.cpu().numpy()) / 24000:.3f} seconds of audio")
...
class Generator:
    def __init__(self, model, processor, device):
        ...
        self.model.generation_config.max_length = 250  # big enough to avoid recompilation
        self.model.generation_config.max_new_tokens = None  # would take precedence over max_length
    ...
    def _generate_stream(
        ...
        **kwargs,
        ...
        model_kwargs = self.model._get_initial_cache_position(cur_len, device, model_kwargs)
        ...
        depth_decoder_input_ids = torch.nn.functional.pad(first_codebook_ids, (1, 0), value=0)
        ...
        while not eos_generated:
            ...
            if input_ids.dim() == 2:  # [batch, seq_len]
                ...
                if codebook_ids.size(1) < num_codebooks:
                    ...
                    padding = torch.full((1, num_codebooks - codebook_ids.size(1)), 
                                        eos_id, dtype=codebook_ids.dtype, device=device)
                    codebook_ids_padded = torch.cat([codebook_ids, padding], dim=1)
                    next_input_ids = codebook_ids_padded.unsqueeze(1)  # [batch, 1, num_codebooks]
                else:
                    next_input_ids = codebook_ids.unsqueeze(1)  # [batch, 1, num_codebooks]
            else:
                ...
                if codebook_ids.size(1) < num_codebooks:
                    padding = torch.full((1, num_codebooks - codebook_ids.size(1)), 
                                        eos_id, dtype=codebook_ids.dtype, device=device)
                    codebook_ids_padded = torch.cat([codebook_ids, padding], dim=1)
                    next_codebook_ids = codebook_ids_padded.unsqueeze(1)  # [batch, 1, num_codebooks]
                else:
                    next_codebook_ids = codebook_ids.unsqueeze(1)  # [batch, 1, num_codebooks]
            ...
            if token_count >= chunk_token_size or eos_generated:
                ...
                if max_len > 0:  # Only if we have tokens to process
                    audio_tensor = torch.full((1, num_codebooks, max_len), 
                                             eos_id, dtype=torch.long, device=device)
                    ...
                    with torch.no_grad():
                        decoded_audio = self.model.codec_model.decode(audio_tensor).audio_values
                        ...
                        chunk_rtf = chunk_gen_time / audio_duration if audio_duration > 0 else 0
                        ...
                        avg_rtf = (chunk_end_time - start_time_total) / total_audio_duration if total_audio_duration > 0 else 0
                        
                        # Calculate first chunk latency (if this is the first chunk)
                        if not first_chunk_generated:
                            first_chunk_latency = chunk_end_time - first_chunk_start_time
                            print(f"First chunk latency: {first_chunk_latency:.3f}s")
                            first_chunk_generated = True
                        
                        print(f"Chunk {chunk_count}: RTF = {chunk_rtf:.4f} (chunk duration: {audio_duration:.3f}s, gen time: {chunk_gen_time:.3f}s)")
                        print(f"Running average RTF: {avg_rtf:.4f}")
                        
                        yield chunk_audio
                    
                    # Reset token collection except for the last token (for continuity)
                    last_tokens = [tokens[-1] if tokens else eos_id for tokens in audio_codebooks]
                    audio_codebooks = [[] for _ in range(num_codebooks)]
        ...
        if chunk_count > 0:
            ...
            final_rtf = total_time / total_audio_duration if total_audio_duration > 0 else 0
...
def generate_streaming_audio(
    ...
    **kwargs : dict
    ...
    try:
        ...
        if reference_data:
            print(f"Adding {len(reference_data)} reference audio sample(s) to conversation")
            for ref in reference_data:
                ...
                if audio_array is None or not isinstance(audio_array, np.ndarray) or audio_array.size == 0:
                ...
                print(f"Adding reference audio: speaker={speaker_id}, text='{text[:30]}...', "
        ...
        print(f"Applying chat template to conversation with {len(final_conversation)} entries")
        try:
            inputs = generator.processor.apply_chat_template(
                final_conversation,
                tokenize=True,
            ...
            print(f"Generated {inputs['input_ids'].shape[1]} tokens for the conversation")
        ...
        if output_filename:
            wav_fh = sf.SoundFile(output_filename, "w", samplerate=24000, channels=1, subtype="PCM_16")
        else:
            wav_fh = None
        ...
        for pcm_chunk in generator.generate_stream(inputs, chunk_token_size=chunk_token_size, **kwargs):
...
def load_reference_audio(reference_data):
    ...
    for ref in reference_data:
        ...
        try:
            ...
            print(f"Audio range: min={ref_copy['audio_array'].min():.4f}, max={ref_copy['audio_array'].max():.4f}")
